...
Get-ChildItem src/before_pub/*.md | ForEach-Object {
   ...
   pandoc -f markdown -t html --template=src/templates/template.html --preserve-tabs --eol=lf --toc --no-highlight -V pagetitle=$filename --mathjax $_.FullName > "site/items/$filename"
...
Get-ChildItem src/items/with-katex/*.md | ForEach-Object {
   $filename = "$($_.BaseName).html"
   pandoc -f markdown -t html5 --template=src/templates/template-with-katex.html --preserve-tabs --eol=lf --toc --no-highlight -V pagetitle=$filename --mathjax $_.FullName > "site/items/$filename"
   Write-Output $filename
}
...
Copy-Item -Recurse -Force img site/img
